...
def _get_extra_feature_group_performances(factory, trainer, parse_fn, extra_groups, feature_to_type, record_count):
  ...
  for group_name, raw_feature_regex_list in extra_groups.items():
    ...
    fnames = match_feature_regex_list(
        ...
        feature_regex_list=list(raw_feature_regex_list),
        preprocess=False,
        as_dict=False,
    )
...
def _feature_importances_tree_algorithm(
  ...
  individual_feature_performance_results = dict(
      out, **dict(individual_feature_performances))
  group_feature_performance_results = dict(feature_group_performances)
...
def compute_feature_importances(
  ...
  if not feature_config:
    ...
    return algorithm(data_dir=data_dir,
                     trainer=trainer,
                     parse_fn=parse_fn,
                     fnames=fnames,
                     file_list=file_list,
...
def write_feature_importances_to_hdfs(
  ...
  if perfs:
    ...
    out.extend("{name}\t{drop}\t{pdrop}%\t{perf}".format(**entry)
...
def write_feature_importances_to_ml_dash(trainer, feature_importances, feature_config=None):
  ...
  """
  experiment_tracking_path = (trainer.experiment_tracker.tracking_path or
                              ExperimentTracker.guess_path(trainer._save_dir))

  logging.info(
      f'Computing feature importances for run: {experiment_tracking_path}')
    ...
    for feature, imps in feature_importances[key].items():
      ...
      for metric_name, value in imps.items():
        try:
          ...
          logging.info(
              f'Wrote feature importance value {str(value)} for metric: {metric_name}'
          )
        except Exception as ex:
          logging.error(
              f"Skipping writing metric:{metric_name} to ML Metastore due to invalid metric value: {str(value)} or value type: {type(value)}. Exception: {str(ex)}"
          )
...
# setting feature config to match the one used in compute_feature_importances
  ...
  try:
    ...
    logging.error(
        f'Feature importance is not being written due to: HTTPError when attempting to write to ML Metastore: \n{err}.'
    )
